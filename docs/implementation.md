...
| 模块 | 职责 | 对应测试 |
| --- | --- | --- |
| `src/pgscope_mcp/server.py` | 注册五个 MCP 工具，校验参数，封装结构化结果与脱敏错误 | `tests/test_mcp.py` |
| `src/pgscope_mcp/config.py` | 从环境变量读取服务端配置，限制 schema、超时与实测权限 | `tests/test_config_results.py` |
| `src/pgscope_mcp/validation.py` | 解析 PostgreSQL SQL，递归检查 AST，将真实表绑定到授权 schema | `tests/test_validation.py` |
| `src/pgscope_mcp/database.py` | 获取表元数据，管理只读事务，执行查询与 JSON 执行计划 | `tests/test_database.py` |
| `src/pgscope_mcp/diagnostics.py` | 汇总计划节点，结合 AST、元数据与计划生成六类诊断 | `tests/test_diagnostics.py` |
| `src/pgscope_mcp/results.py` | JSON 序列化与查询响应大小控制 | `tests/test_config_results.py` |
| `src/pgscope_mcp/models.py` | 定义 SQL 校验结果、表引用和可公开错误 | 由上述模块测试覆盖 |
| `scripts/benchmark.py` | 在专用演示库执行两组优化实验，保存计划与测量数据 | `tests/test_benchmark.py` |
| `scripts/smoke_mcp.py` | 通过 stdio 初始化服务并调用五个工具 | 独立客户端冒烟验证 |
...
开发命令见 [贡献指南](../CONTRIBUTING.md)，已执行的验收及其环境见 [验证记录](validation.md)。优化实验保留原始计划与五次测量值，以中位数汇总；不同环境应重新测量，不把已有结果作为性能承诺。
